...
exports.handle = async (req, res) => {
  try {
    ...
    if (fullname < 5 || short_name < 5) {
      ...
      const ress = await cabinet.addNewSchool({
        fullname,
        short_name,
        area_id,
        type_id,
...
exports.getSchoolProfileByIdWithFILTERcurrentProject = async (req, res) => {
  try {
    ...
    const teachers = await Teacher.getAllTeachersFromThisSchoolFromCurrentProject(
      req.params
    );
...
exports.getProfileByTeacherIdInProject = async (req, res) => {
  try {
    ...
    const bdresult =
      (await teacher[0].birthday.getDate()) +
      " " +
      month[teacher[0].birthday.getMonth()] +
      " " +
      teacher[0].birthday.getFullYear();
...
exports.getProfileByTeacherId = async (req, res) => {
  try {
    ...
    const discipline = await cabinet.getTeacherDisciplines(params);
    const project_id = await req.params.project_id;
    const issetInProjects = await cabinet.getInformationAboutIssetTeacherInProject(
      req.params
    );
...
exports.getSchoolsTeacherLibrary3 = async (req, res) => {
  try {
    ...
    const common_data = await Teacher.getAllTeachersNotMembersInCurrentProject(
      id
    );

    const members_current_project_teachers = await Teacher.getAllTeachersFromThisSchoolFromCurrentProject(
      req.params
    );
...
exports.select_school_for_add_in_project = async (req, res) => {
  try {
    ...
    const current_project_data = await cabinet.getAllSchoolsFromThisProjects(
      req.params
    );
    ...
    const this_project_information = await cabinet.getInfoFromProjectById(
      project_id
    );

    //общее кол-во учителей
    const total_value_teacher_current_project = await cabinet.getTotalValueTeacherFromProject(
      req.params
    );
    ...
    //общее кол-во школ
    const total_value_schools_current_project = await cabinet.getTotalValueSchoolFromProject(
      req.params
    );
    ...
    const school_isset_in_project = await cabinet.getSchoolFromCurrentProject(
      req.body
    );
...
exports.changeStatusSchool = async (req, res) => {
  try {
    ...
    if (changeStatusSchool.affectedRows) {
      ...
      return res
        .status(200)
        .redirect("/admin/projects/" + req.params.project_id + "/schools");
